...
if (isset($_POST['log']))
{
  ...
  if(empty($_POST['username']) || empty($_POST['password'])   )
  {
  	echo "<script>alert('You have an empty field.');</script>";
      	...
      	echo"<script>location.assign('AdminLogin.php')</script>";  // go back to the login page
  }
      ...
      $mysql_result= mysqli_query($link,"SELECT * from log_admin WHERE admin_name LIKE '$username' AND password LIKE '$password' ");
...
echo $row_cnt;
			...
			if($row_cnt!=1)   // if there is no such user like that.
		{
      	...
      	echo"<script>location.assign('admin.html')</script>";  // go to the new page
...
<html>
		...
		<body style='width: 100% ; height:100% ; position: fixed; background-color: yellow ; color: black;'>
			...
			<form action='AdminLogin.php' method=POST style='padding: 225px 550px 100px' ;>
